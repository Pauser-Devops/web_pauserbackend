...
router.put("/:id", auth_ts_1.authMiddleware, async (req, res) => {
    try {
        ...
        const userId = (0, frequency_ts_1.parseId)(id);
        const existingUser = await prisma_ts_1.prisma.user.findUnique({ where: { id: userId } });
        if (!existingUser) {
        ...
        const user = await prisma_ts_1.prisma.user.update({
            where: { id: userId },
            data: updateData,
            include: {
                role: { select: { id: true, name: true } },
...
router.delete("/:id", auth_ts_1.authMiddleware, async (req, res) => {
    try {
        ...
        await prisma_ts_1.prisma.user.delete({ where: { id: userId } });
...
router.get("/roles", auth_ts_1.authMiddleware, async (req, res) => {
    try {
        const roles = await prisma_ts_1.prisma.role.findMany();
        res.json(roles);
    }
    catch (error) {
...
router.put("/:id/role", auth_ts_1.authMiddleware, async (req, res) => {
    try {
        if (req.user?.roleId !== 1) {
            return res.status(403).json({ error: "Solo admins pueden modificar usuarios" });
        }
...
router.get("/", auth_ts_1.authMiddleware, async (req, res) => {
    try {
        ...
        const where = {};
        if (sedeId)
            where.sedeId = (0, frequency_ts_1.queryNum)(sedeId);
        if (unidadId)
            where.unidadId = (0, frequency_ts_1.queryNum)(unidadId);
        if (cargoId)
            where.cargoId = (0, frequency_ts_1.queryNum)(cargoId);
        const users = await prisma_ts_1.prisma.user.findMany({
            where,
            select: {
                id: true,
                email: true,
...
router.put("/profile", auth_ts_1.authMiddleware, async (req, res) => {
    try {
...
router.get("/:id/programs", auth_ts_1.authMiddleware, async (req, res) => {
    try {
        if (req.user?.roleId !== 1) {
            return res.status(403).json({ error: "Solo admins pueden ver programas de usuario" });
        }
        const userId = (0, frequency_ts_1.parseId)(req.params.id);
        ...
        const allPrograms = await prisma_ts_1.prisma.program.findMany({
            where: { isActive: true },
            select: {
                id: true,
                name: true,
...
router.post("/:id/programs", auth_ts_1.authMiddleware, async (req, res) => {
    try {
        if (req.user?.roleId !== 1) {
        ...
        const user = await prisma_ts_1.prisma.user.findUnique({ where: { id: userId } });
        if (!user) {
            return res.status(404).json({ error: "Usuario no encontrado" });
        }
        // Obtener asignaciones actuales
        const currentAssignments = await prisma_ts_1.prisma.userProgram.findMany({
            where: { userId },
        ...
        // Ejecutar cambios
        const results = await prisma_ts_1.prisma.$transaction(async (tx) => {
            // Agregar nuevos
            if (toAdd.length > 0) {
