...
function getCellText(cell) {
    ...
    if (typeof v === "object" && "result" in v) {
        return getCellTextFromValue(v.result);
    }
    return String(v).trim();
}
function getCellTextFromValue(v) {
    if (v instanceof Date)
        return formatDate(v);
    if (v === null || v === undefined)
        return "";
...
async function parseExcelRuta(buffer, period) {
...
router.get("/:programId/config", auth_ts_1.authMiddleware, async (req, res) => {
    try {
        const programId = parseId(req.params.programId);
        const config = await prisma_ts_1.prisma.programFileConfig.findUnique({
            where: { programId },
...
router.post("/:programId/upload-template", auth_ts_1.authMiddleware, upload.single("file"), async (req, res) => {
    try {
        ...
        const mergedCells = sheet.model.merges?.map((m) => ({ range: m })) || [];
        const checksum = {
...
router.put("/:programId/config", auth_ts_1.authMiddleware, async (req, res) => {
    try {
        if (req.user?.roleId !== 1) {
        ...
        const config = await prisma_ts_1.prisma.programFileConfig.upsert({
            where: { programId },
            create: {
                programId,
        ...
        const excelQuestion = await prisma_ts_1.prisma.question.findFirst({
        ...
        if (excelQuestion) {
            ...
            await prisma_ts_1.prisma.question.create({
                data: {
...
router.get("/:programId/records", auth_ts_1.authMiddleware, async (req, res) => {
    try {
        const programId = parseId(req.params.programId);
        const config = await prisma_ts_1.prisma.programFileConfig.findUnique({
            where: { programId },
...
router.get("/:programId/records/latest", auth_ts_1.authMiddleware, async (req, res) => {
    try {
        const programId = parseId(req.params.programId);
        const config = await prisma_ts_1.prisma.programFileConfig.findUnique({ where: { programId } });
        if (!config) {
            return res.json(null);
        }
        const latestRecord = await prisma_ts_1.prisma.monthlyRecord.findFirst({
...
router.get("/:programId/records/:period", auth_ts_1.authMiddleware, async (req, res) => {
    try {
        const programId = parseId(req.params.programId);
        ...
        const record = await prisma_ts_1.prisma.monthlyRecord.findUnique({
...
router.patch("/:programId/records/:period/ajuste", auth_ts_1.authMiddleware, async (req, res) => {
    try {
        if (req.user?.roleId !== 1) {
        ...
        await prisma_ts_1.prisma.monthlyRecord.update({
            where: { id: record.id },
...
router.post("/:programId/records/:period", auth_ts_1.authMiddleware, async (req, res) => {
    try {
        if (req.user?.roleId !== 1) {
        ...
        const record = await prisma_ts_1.prisma.monthlyRecord.upsert({
            where: { configId_period: { configId: config.id, period } },
            create: {
                configId: config.id,
                period,
...
router.post("/:programId/records/:period/import", auth_ts_1.authMiddleware, upload.single("file"), async (req, res) => {
    try {
        if (req.user?.roleId !== 1) {
...
router.get("/:programId/pending-status", auth_ts_1.authMiddleware, async (req, res) => {
    try {
        const programId = parseId(req.params.programId);
        const config = await prisma_ts_1.prisma.programFileConfig.findUnique({ where: { programId } });
        ...
        const existingRecord = await prisma_ts_1.prisma.monthlyRecord.findUnique({
            where: { configId_period: { configId: config.id, period } },
        });
...
router.get("/:programId/records/:period/summary", auth_ts_1.authMiddleware, async (req, res) => {
    try {
        const programId = parseId(req.params.programId);
        const period = paramStr(req.params.period);
        const config = await prisma_ts_1.prisma.programFileConfig.findUnique({ where: { programId } });
...
//# sourceMappingURL=programFiles.js.map