...
exports.paramNumber = paramNumber;
exports.queryNum = queryNum;
exports.getCurrentPeriod = getCurrentPeriod;
exports.isQuestionAvailableToday = isQuestionAvailableToday;
/**
 * Convierte req.params.id de Express 5.x (string | string[]) a string
 */
function paramString(val) {
    if (!val)
        return "";
 ...
 */
exports.parseId = paramNumber;
/**
 * Convierte query param de Express 5.x a número
 */
function queryNum(val) {
    if (!val)
        return null;
    if (Array.isArray(val))
        return paramNumber(val[0]);
    if (typeof val === "object") {
        const first = Object.values(val)[0];
        if (Array.isArray(first))
            return paramNumber(first[0]);
        return paramNumber(first);
    }
    return paramNumber(val);
}
/**
 ...
 */
function getCurrentPeriod(frequencyType, frequencyDay, frequencyInterval, referenceDate = new Date()) {
    ...
    switch (frequencyType) {
        ...
        case "SEMANAL": {
            ...
            const weeksSinceYearStart = Math.floor((weekStart.getTime() - startOfYear.getTime()) / (7 * 24 * 60 * 60 * 1000));
 ...
 */
function isQuestionAvailableToday(frequencyType, frequencyDay, frequencyInterval, today = new Date()) {
    switch (frequencyType) {
        case "DIARIA":
        ...
        case "SEMANAL": {
            ...
            const weeksSinceYearStart = Math.floor((today.getTime() - startOfYear.getTime()) / msInWeek);
